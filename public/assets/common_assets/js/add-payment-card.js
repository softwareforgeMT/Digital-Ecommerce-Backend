...
function initializeStripeCard() {
    ...
    document.getElementById('stripeForm').addEventListener('click',  async (event) => {
        if (event.target.id === 'card-button') {
            ...
            var fullNameInput = document.getElementById('cc-name');
            var full_name_user = fullNameInput.value;
            var displayError = document.getElementById('card-errors');

            button.attr({ 'disabled': 'true' });
            button.find('i').removeClass('ri-shopping-basket-line label-icon align-middle fs-16 ms-2');
            button.find('i').addClass('spinner-border spinner-border-sm align-middle me-1');
            //For singlepayments
