...
function loadProductData(datas) {
  ...
  Array.from(datas).forEach(function (prodctData, index) {
    ...
    document.querySelector("#explorecard-list").innerHTML += '<div class="col list-element">\
...
productCategoryInput.passedElement.element.addEventListener("change", function (event) {
  ...
  if (event.detail.value) {
    var filterData = allproductlist.filter(function (productlist) {
      return productlist.category === productCategoryValue;
    });
    if (filterData.length == 0) {
      document.getElementById("noresult").style.display = "block";
      document.getElementById("loadmore").style.display = "none";
...
productFileTypeInput.passedElement.element.addEventListener("change", function (event) {
  ...
  if (event.detail.value) {
    var filterData = allproductlist.filter(function (productlist) {
      return productlist.productImg.split(".").pop() == productFileTypeValue;
    });
    if (filterData.length == 0) {
      document.getElementById("noresult").style.display = "block";
      document.getElementById("loadmore").style.display = "none";
...
productSalesInputInput.passedElement.element.addEventListener("change", function (event) {
  ...
  if (event.detail.value) {
    var filterData = allproductlist.filter(function (productlist) {
      return productlist.salesType === productCategoryValue;
    });
    if (filterData.length == 0) {
      document.getElementById("noresult").style.display = "block";
      document.getElementById("loadmore").style.display = "none";
...
rangeProductPrice.noUiSlider.on("change", function (values, handle) {
  ...
  var filterDataAll = productListupdatedAll.filter(function (product) {
...
function mergeTooltips(slider, threshold, separator) {
  ...
  if (slider) slider.noUiSlider.on("update", function (values, handle, unencoded, tap, positions) {
    var pools = [[]];
    var poolPositions = [[]];
    var poolValues = [[]];
    var atPool = 0;
...
function loadMoreBtn() {
  ...
  if (loadmore) {
    ...
    loadmore.addEventListener("click", function (e) {
      var elementList = [].concat(_toConsumableArray(document.querySelectorAll("#explorecard-list .list-element")));
      if (elementList) {
