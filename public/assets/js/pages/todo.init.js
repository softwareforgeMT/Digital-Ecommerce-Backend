/******/ (() => { // webpackBootstrap
/*!*****************************************!*\
  !*** ./resources/js/pages/todo.init.js ***!
  \*****************************************/
...
if (addNewProject) {
  document.getElementById("addNewProject").addEventListener("click", function () {
    ...
    projectlisthtml = '<li id="projectlist-' + uniqueid + '">\
  ...
  Array.from(elem.querySelectorAll(".dropdown-menu ul li a")).forEach(function (subElem) {
    subElem.addEventListener("click", function () {
      ...
      if (subElem.classList.contains("active")) {
        ...
        folderlisthtml = '<a href="javascript: void(0);" class="avatar-group-item mb-2" data-img="' + imgPath + '"  data-bs-toggle="tooltip" data -bs-placement="top" title="' + nameelem + '">\
...
document.getElementById("createTask").addEventListener("hidden.bs.modal", function (event) {
  clearFields();
});
...
function drawList(manyTodos) {
  ...
  Array.from(manyTodos).forEach(function (singleTodo) {
    ...
    Array.from(singleTodo.assignedto).forEach(function (img) {
      imgHtml += "\n                <a href=\"javascript: void(0);\" class=\"avatar-group-item\" data-img=\"".concat(img.assigneeImg, "\"  data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" title=\"").concat(img.assigneeName, "\">\n                    <img src=\"").concat(img.assigneeImg, "\" alt=\"\" class=\"rounded-circle avatar-xxs\" />\n                </a>\n            ");
    });
    imgHtml += "</div>";
    document.getElementById("task-list").innerHTML += '<tr>\
            <td>\
          ...
          Array.from(document.querySelectorAll(".select-element .dropdown-menu ul li a")).forEach(function (subElem) {
            ...
            item.assignedto.map(function (subItem) {
              if (subItem.assigneeName == nameelem) {
                ...
                var folderListdata = document.getElementById("assignee-member");
                if (subElem.classList.contains("active")) {
                  folderlisthtml = '<a href="javascript: void(0);" class="avatar-group-item mb-2" data-img="' + subItem.assigneeImg + '"  data-bs-toggle="tooltip" data -bs-placement="top" title="' + subItem.assigneeName + '">\
...
taskStatusInput.passedElement.element.addEventListener("change", function (event) {
  ...
  if (event.detail.value) {
    var filterData = todoList.filter(function (taskList) {
      return taskList.status == taskStatusValue;
    });
    if (filterData.length == 0) {
      document.getElementById("noresult").style.display = "block";
      document.getElementById("todo-task").style.display = "none";
