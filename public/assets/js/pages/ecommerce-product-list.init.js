...
var productListAll = new gridjs.Grid({
  columns: [{
    ...
    width: "360px",
    data: function data(row) {
      return gridjs.html('<div class="d-flex align-items-center">' + '<div class="flex-shrink-0 me-3">' + '<div class="avatar-sm bg-light rounded p-1"><img src="' + row.product.img + '" alt="" class="img-fluid d-block"></div>' + "</div>" + '<div class="flex-grow-1">' + '<h5 class="fs-14 mb-1"><a href="apps-ecommerce-product-details" class="text-dark">' + row.product.title + "</a></h5>" + '<p class="text-muted mb-0">Category : <span class="fw-medium">' + row.product.category + "</span></p>" + "</div>" + "</div>");
    }
  }, {
    name: "Stock",
    width: "94px"
    ...
    formatter: function formatter(cell, row) {
      ...
      return gridjs.html('<div class="dropdown">' + '<button class="btn btn-soft-secondary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">' + '<i class="ri-more-fill"></i>' + "</button>" + '<ul class="dropdown-menu dropdown-menu-end">' + '<li><a class="dropdown-item" href="apps-ecommerce-product-details"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a></li>' + '<li><a class="dropdown-item edit-list" data-edit-id=' + x + ' href="apps-ecommerce-add-product"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>' + '<li class="dropdown-divider"></li>' + '<li><a class="dropdown-item remove-list" href="#" data-id=' + x + ' data-bs-toggle="modal" data-bs-target="#removeItemModal"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>' + "</ul>" + "</div>");
    }
  }],
  className: {
    th: "text-muted"
...
var productListPublished = new gridjs.Grid({
  columns: [{
    ...
    formatter: function formatter(cell, row) {
      return gridjs.html('<div class="dropdown">' + '<button class="btn btn-soft-secondary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">' + '<i class="ri-more-fill"></i>' + "</button>" + '<ul class="dropdown-menu dropdown-menu-end">' + '<li><a class="dropdown-item" href="apps-ecommerce-product-details"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a></li>' + '<li><a class="dropdown-item" href="apps-ecommerce-add-product"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>' + '<li class="dropdown-divider"></li>' + '<li><a class="dropdown-item remove-list" href="#" data-id=' + row._cells[0].data + ' data-bs-toggle="modal" data-bs-target="#removeItemModal"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>' + "</ul>" + "</div>");
    }
  }],
  className: {
    th: "text-muted"
...
Array.from(document.querySelectorAll(".filter-list a")).forEach(function (filteritem) {
  filteritem.addEventListener("click", function () {
    ...
    var filterData = productListAllData.filter(function (filterlist) {
      return filterlist.product.category === filterItemValue;
    });
    var filterPublishedData = productListPublishedData.filter(function (filterlist) {
      return filterlist.product.category === filterItemValue;
    });
    productListAll.updateConfig({
      data: filterData
...
slider.noUiSlider.on("update", function (values, handle) {
  ...
  filterDataAll = productListupdatedAll.filter(function (product) {
    return parseFloat(product.price) >= minvalue && parseFloat(product.price) <= maxvalue;
  });
  filterDataPublished = productListupdatedPublished.filter(function (product) {
    return parseFloat(product.price) >= minvalue && parseFloat(product.price) <= maxvalue;
  });
  productListAll.updateConfig({
    data: filterDataAll
...
function removeItems() {
  ...
  removeItem.addEventListener("show.bs.modal", function (event) {
    ...
    document.getElementById("delete-product").addEventListener("click", function () {
      ...
      if (document.getElementById("selection-element")) document.getElementById("selection-element").style.display = "none";
