...
var url = "assets/json/";
var allmaillist = "";
var loader = document.querySelector("#elmLoader");
// showing loading
...
function loadMailData(datas) {
  ...
  Array.from(datas).forEach(function (mailData, index) {
    ...
    document.querySelector("#mail-list").innerHTML += '<li class="' + checkReaded + '">\
...
function loadSocialMailData(datas) {
  Array.from(datas).forEach(function (mailData, index) {
    ...
    var checkStarred = mailData.starred ? "active" : "";
    var mailcounted = mailData.counted ? "(" + mailData.counted + ")" : "";
    document.getElementById("social-mail-list").innerHTML += '<li class="' + checkReaded + '">\
                <div class="col-mail col-mail-1">\
                    <div class="form-check checkbox-wrapper-mail fs-14">\
                        <input class="form-check-input" type="checkbox" value="' + mailData.id + '" id="checkbox-' + mailData.id + '">\
                        <label class="form-check-label" for="checkbox-' + mailData.id + '"></label>\
...
function loadPromotionsMailData(datas) {
  Array.from(datas).forEach(function (mailData, index) {
    ...
    var checkStarred = mailData.starred ? "active" : "";
    var mailcounted = mailData.counted ? "(" + mailData.counted + ")" : "";
    document.getElementById("promotions-mail-list").innerHTML += '<li class="' + checkReaded + '">\
                <div class="col-mail col-mail-1">\
                    <div class="form-check checkbox-wrapper-mail fs-14">\
                        <input class="form-check-input" type="checkbox" value="' + mailData.id + '" id="checkbox-' + mailData.id + '">\
                        <label class="form-check-label" for="checkbox-' + mailData.id + '"></label>\
...
Array.from(document.querySelectorAll(".mail-list a")).forEach(function (mailTab) {
  mailTab.addEventListener("click", function () {
    ...
    if (mailTab.querySelector(".mail-list-link").hasAttribute("data-type")) {
      ...
      var filterData = allmaillist.filter(function (maillist) {
        return maillist.labeltype === tabname;
      });
    } else {
      var tabname = mailTab.querySelector(".mail-list-link").innerHTML;
      ...
      if (tabname != "All") {
        var filterData = allmaillist.filter(function (maillist) {
          return maillist.tabtype === tabname;
        });
      } else {
...
function scrollToBottom(id) {
  setTimeout(function () {
    ...
    var offsetHeight = document.getElementsByClassName("chat-conversation-list")[0] ? document.getElementById(id).getElementsByClassName("chat-conversation-list")[0].scrollHeight - window.innerHeight + 750 : 0;
...
function removeItems() {
  ...
  removeItem.addEventListener("show.bs.modal", function (event) {
    document.getElementById("delete-record").addEventListener("click", function () {
      ...
      if (document.getElementById("email-topbar-actions")) document.getElementById("email-topbar-actions").style.display = "none";
...
markAllReadBtn.addEventListener("click", function (event) {
  if (document.querySelectorAll(".message-list li.unread").length === 0) {
    var hideclipboardNew = function hideclipboardNew() {
