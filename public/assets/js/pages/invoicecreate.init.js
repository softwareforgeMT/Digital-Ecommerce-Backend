...
function new_link() {
    ...
    var delLink =
        ...
        '<td class="text-start">' +
        '<div class="mb-2">' +
        '<input class="form-control bg-light border-0" type="text" id="productName-' +
        count +
        ...
        "</td>" +
        "<td>" +
        '<input class="form-control bg-light border-0 product-price" type="number" id="productRate-' +
        count +
        ...
        '<td class="text-end">' +
        "<div>" +
        '<input type="text" class="form-control bg-light border-0 product-line-price" id="productPrice-' +
        count +
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formEvent.addEventListener("submit", function (event) {
        ...
        var invoice_amount = document
            .getElementById("totalamountInput")
            .value.slice(1);
        var status = document.getElementById("choices-payment-status").value;
        var billing_address_full_name =
            document.getElementById("billingName").value;
        var billing_address_address =
            document.getElementById("billingAddress").value;
        var billing_address_phone = document
            .getElementById("billingPhoneno")
            .value.replace(/[^0-9]/g, "");
        var billing_address_tax = document.getElementById("billingTaxno").value;
        var shipping_address_full_name =
            document.getElementById("shippingName").value;
        var shipping_address_address =
            document.getElementById("shippingAddress").value;
        var shipping_address_phone = document
            .getElementById("shippingPhoneno")
            .value.replace(/[^0-9]/g, "");
        var shipping_address_tax =
            document.getElementById("shippingTaxno").value;
        var payment_details_payment_method = document.getElementById(
            "choices-payment-type"
        ).value;
        var payment_details_card_holder_name =
            document.getElementById("cardholderName").value;
        var payment_details_card_number = document
            .getElementById("cardNumber")
            .value.replace(/[^0-9]/g, "");
        var payment_details_total_amount = document
            .getElementById("amountTotalPay")
            .value.slice(1);
        var company_details_legal_registration_no = document
            .getElementById("registrationNumber")
            .value.replace(/[^0-9]/g, "");
        var company_details_email =
            document.getElementById("companyEmail").value;
        var company_details_website =
            document.getElementById("companyWebsite").value;
        var company_details_contact_no = document
            .getElementById("compnayContactno")
            .value.replace(/[^0-9]/g, "");
        var company_details_address =
            document.getElementById("companyAddress").value;
        var company_details_zip_code = document.getElementById(
            "companyaddpostalcode"
        ).value;
        var order_summary_sub_total = document
            .getElementById("cart-subtotal")
            .value.slice(1);
        var order_summary_estimated_tex = document
            .getElementById("cart-tax")
            .value.slice(1);
        var order_summary_discount = document
            .getElementById("cart-discount")
            .value.slice(1);
        var order_summary_shipping_charge = document
            .getElementById("cart-shipping")
            .value.slice(1);
        var order_summary_total_amount = document
        ...
        Array.from(products).forEach((element) => {
            var product_name = element.querySelector(
                "#productName-" + count
            ).value;
            var product_details = element.querySelector(
                "#productDetails-" + count
            ).value;
            var product_rate = parseInt(
                element.querySelector("#productRate-" + count).value
            );
            var product_qty = parseInt(
                element.querySelector("#product-qty-" + count).value
            );
            var product_price = element
                .querySelector("#productPrice-" + count)
                .value.split("$");
            ...
            if (options == "edit-invoice" && invoice_no == i_no) {
                ...
                invoices[objIndex].company_details = {
                    legal_registration_no:
                        company_details_legal_registration_no,
                ...
                localStorage.setItem(
                    "new_data_object",
                    JSON.stringify(new_data_object)
                );
            }
