...
class MessagesController extends Controller
{
    ...
    {   
        ...
        $view=Auth::guard('admin')->check()?'admin.livechat.messenger':'user.livechat.show';
        $id2=Auth::guard('admin')->check()?$id:1;
        ...
        $users = User::where(function ($query) {
            $query->whereExists(function ($subquery) {
                $subquery->select(DB::raw(1))
                        ->from('ch_messages')
                        ->whereRaw('ch_messages.from_id = users.id OR ch_messages.to_id = users.id')
                        ->where('users.id', '!=', Auth::user()->id)
    ...
    public function updateLayoutMode(Request $request)
    { 
        ...
        // Return a response indicating success or failure if needed
         return response()->json(['message' => 'Layout mode updated successfully']);
    }
