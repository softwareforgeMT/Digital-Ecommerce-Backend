...
function loadProductData(datas) {
    ...
    Array.from(datas).forEach(function (prodctData, index) {
        ...
        document.querySelector("#explorecard-list").innerHTML +=
            ...
            likeBtn +
            '" data-bs-toggle="button" aria-pressed="true"><i class="mdi mdi-cards-heart fs-16"></i></button>\
                </div>\
                <div class="card-body">\
                    <p class="fw-medium mb-0 float-end"><i class="mdi mdi-heart text-danger align-middle"></i> ' +
            prodctData.totalLikes +
            ' </p>\
                    <h5 class="mb-1"><a href="apps-nft-item-details">' +
            prodctData.title +
            '</a></h5>\
                ...
                <div class="card-footer border-top border-top-dashed">\
                    <div class="d-flex align-items-center">\
                        <div class="flex-grow-1 fs-14">\
                            <i class="ri-price-tag-3-fill text-warning align-bottom me-1"></i> Highest: <span class="fw-medium">' +
            prodctData.highBid +
            '</span>\
                        </div>\
...
productCategoryInput.passedElement.element.addEventListener(
    ...
    function (event) {
        ...
        if (event.detail.value) {
            var filterData = allproductlist.filter(
...
productFileTypeInput.passedElement.element.addEventListener(
    ...
    function (event) {
        ...
        if (event.detail.value) {
            var filterData = allproductlist.filter(
                (productlist) =>
                    productlist.productImg.split(".").pop() ==
                    productFileTypeValue
            );
            if (filterData.length == 0) {
                document.getElementById("noresult").style.display = "block";
                document.getElementById("loadmore").style.display = "none";
...
productSalesInputInput.passedElement.element.addEventListener(
    ...
    function (event) {
        ...
        if (event.detail.value) {
            var filterData = allproductlist.filter(
                (productlist) => productlist.salesType === productCategoryValue
            );
            if (filterData.length == 0) {
                document.getElementById("noresult").style.display = "block";
                document.getElementById("loadmore").style.display = "none";
                ...
                Array.from(pools).forEach(function (pool, poolIndex) {
                    ...
                    for (var j = 0; j < handlesInPool; j++) {
                        ...
                        if (j === handlesInPool - 1) {
                            ...
                            Array.from(poolPositions[poolIndex]).forEach(
                                function (value) {
                                    offset += 1000 - value;
                                }
                            );
