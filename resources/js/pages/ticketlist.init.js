...
// Init list
var ticketsList = new List("ticketsList", options).on(
    "updated",
    function (list) {
...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (element) {
        ticketsList.add({
            id:
                '<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="' +
                element.id +
                '" class="fw-medium link-primary ticket-id">#VLZ' +
                element.id +
                "</a>",
    ...
    ticketsList.remove(
        "id",
        `<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="001" class="fw-medium link-primary">#VLZ001</a>`
    );
};
...
function SearchData() {
    ...
    ticketsList.filter(function (data) {
        ...
        if (
            new Date(data.values().create_date.slice(0, 12)) >=
                new Date(date1) &&
            new Date(data.values().create_date.slice(0, 12)) <= new Date(date2)
        ) {
            dateFilter = true;
        } else {
            dateFilter = false;
    ...
    if (typeof ids_array !== "undefined" && ids_array.length > 0) {
        if (confirm("Are you sure you want to delete this?")) {
            Array.from(ids_array).forEach(function (id) {
                ticketsList.remove(
                    "id",
                    `<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="${id}" class="fw-medium link-primary ticket-id">#VLZ${id}</a>`
                );
            });
