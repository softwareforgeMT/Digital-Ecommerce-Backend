...
if (addNewProject) {
    document
        ...
        <div class="collapse" id="projectCollapse-' +
            ...
            if (projectName !== "") {
                var projectListdata =
                    document.getElementById("projectlist-data");
                projectListdata.insertAdjacentHTML(
                    "beforeend",
                    projectlisthtml
                );
                var addProjectClose = document.getElementById(
                    "addProjectBtn-close"
                );
...
Array.from(document.getElementsByClassName("createTask")).forEach(function (
    elem
) {
...
Array.from(document.getElementsByClassName("select-element")).forEach(function (
    elem
) {
    Array.from(elem.querySelectorAll(".dropdown-menu ul li a")).forEach(
        function (subElem) {
            subElem.addEventListener("click", function () {
                subElem.classList.toggle("active");
                var assigneeMember = document.querySelectorAll(
                    ".select-element .dropdown-menu .dropdown-item.active"
                );
                document.getElementById("total-assignee").innerHTML =
                    assigneeMember.length;
                ...
                if (subElem.classList.contains("active")) {
                    var nameelem =
                        subElem.querySelector(".flex-grow-1").innerHTML;
                ...
                <img src="' +
                    ...
                    folderListdata.insertAdjacentHTML(
                        "beforeend",
                        folderlisthtml
                    );
                    tooltipElm();
                    ...
                    Array.from(
                        folderListdata.querySelectorAll(".avatar-group-item")
...
document
    ...
    .addEventListener("submit", function (e) {
        ...
        var assignedtousers = [];
        var assignedTo = document.querySelectorAll(
            ".select-element .dropdown-menu .dropdown-item.active"
        );
        ...
        if (
            inputTitle !== "" &&
            statusInputFieldValue !== "" &&
            priorityInputFieldValue !== "" &&
            !editFlag
        ) {
...
function clearFields() {
    ...
    Array.from(
        document.querySelectorAll("#assignee-member .avatar-group-item")
    ).forEach(function (item) {
        item.remove();
    });
...
var scroll = autoScroll([document.querySelector("#todo-content")], {
    margin: 20,
    maxSpeed: 100,
...
function drawList(manyTodos) {
    ...
    Array.from(manyTodos).forEach(function (singleTodo) {
        ...
        document.getElementById("task-list").innerHTML +=
            ...
            "</td>\
            <td>" +
            isStatus(singleTodo.status) +
            "</td>\
            <td>" +
            isPriority(singleTodo.priority) +
            '</td>\
            <td>\
            <div class="hstack gap-2">\
                <button class="btn btn-sm btn-soft-danger remove-list" data-bs-toggle="modal" data-bs-target="#removeTaskItemModal" data-remove-id=' +
            singleTodo.id +
            '><i class="ri-delete-bin-5-fill align-bottom"></i></button>\
                <button class="btn btn-sm btn-soft-info edit-list" data-bs-toggle="modal" data-bs-target="#createTask" data-edit-id=' +
            singleTodo.id +
...
function checkTaskStatus() {
    Array.from(document.querySelectorAll("#task-list tr")).forEach(function (
        item
    ) {
        item.querySelector(".form-check .form-check-input").addEventListener(
            "change",
            function () {
...
function editTodoList() {
    var getEditid = 0;
    Array.from(document.querySelectorAll(".edit-list")).forEach(function (
        elem
    ) {
        elem.addEventListener("click", function (event) {
            ...
            todoList = todoList.map(function (item) {
                if (item.id == getEditid) {
                    editFlag = true;
                    document.getElementById("createTaskLabel").innerHTML =
                        "Edit Task";
                    ...
                    var statusSelec = new DOMParser().parseFromString(
                        item.status,
                        "text/html"
                    ).body;
                    statusVal.setChoiceByValue(statusSelec.innerHTML);

                    var prioritySelec = new DOMParser().parseFromString(
                        item.priority,
                        "text/html"
                    ).body;
                    ...
                    Array.from(
                        document.querySelectorAll(
                            ".select-element .dropdown-menu ul li a"
                        )
                        ...
                        item.assignedto.map(function (subItem) {
                            if (subItem.assigneeName == nameelem) {
                                ...
                                var folderListdata =
                                    document.getElementById("assignee-member");
                                if (subElem.classList.contains("active")) {
                                    folderlisthtml =
                                        '<a href="javascript: void(0);" class="avatar-group-item mb-2" data-img="' +
                                        subItem.assigneeImg +
                    ...
                    var assigneelength = document.querySelectorAll(
                        ".select-element .dropdown-menu .dropdown-item.active"
                    ).length;
                    document.getElementById("total-assignee").innerHTML =
                        assigneelength;
...
function removeSingleItem() {
    ...
    Array.from(document.querySelectorAll(".remove-list")).forEach(function (
        ...
        item.addEventListener("click", function (event) {
            ...
            document
                .getElementById("remove-todoitem")
                .addEventListener("click", function () {
...
var taskStatusInput = new Choices(
    document.getElementById("choices-select-status"),
    {
        searchEnabled: false,
    }
);

taskStatusInput.passedElement.element.addEventListener(
    "change",
    function (event) {
        var taskStatusValue = event.detail.value;
        if (event.detail.value) {
            var filterData = todoList.filter(
                (taskList) => taskList.status == taskStatusValue
            );
            if (filterData.length == 0) {
                document.getElementById("noresult").style.display = "block";
                document.getElementById("todo-task").style.display = "none";
...
// choices category input
var taskSortListInput = new Choices(
    document.getElementById("choices-select-sortlist"),
    {
        searchEnabled: false,
    }
);

taskSortListInput.passedElement.element.addEventListener(
    "change",
    function (event) {
