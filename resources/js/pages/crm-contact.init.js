...
addBtn.addEventListener("click", function (e) {
    if (
        ...
        contactList.add({
            ...
            <div class="flex-shrink-0"><img src="' +
                customerImg.src +
                '" alt="" class="avatar-xs rounded-circle object-cover"></div>\
            <div class="flex-grow-1 ms-2 name">' +
                customerNameField.value +
                "</div>\
            </div>",
    ...
    Array.from(editBtns).forEach(function (btn) {
        btn.addEventListener("click", function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                var selectedid = isid.body.firstElementChild.innerHTML;
                var tagBadge = new DOMParser()
                    .parseFromString(x._values.tags, "text/html")
                    .body.querySelectorAll("span.badge");
                if (selectedid == itemId) {
                    idField.value = selectedid;
                    customerImg.src = new DOMParser()
                        .parseFromString(x._values.name, "text/html")
                        .body.querySelector("img").src;
                    customerNameField.value = new DOMParser()
                        .parseFromString(x._values.name, "text/html")
        ...
        btn.addEventListener("click", function (e) {
            ...
            Array.from(itemValues).forEach(function (x) {
                ...
                if (selectedid == itemId) {
                    ...
                    document.getElementById("contact-view-detail").innerHTML =
                        codeBlock;
                }
            });
        });
