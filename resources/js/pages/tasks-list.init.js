...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (raw) {
        ...
        tasksList.add({
            id:
                '<a href="apps-tasks-details" class="fw-medium link-primary">#VLZ' +
                raw.id +
                "</a>",
            project_name:
                '<a href="apps-projects-overview" class="fw-medium link-primary">' +
                raw.project_name +
                "</a>",
    ...
    tasksList.remove(
        "id",
        `<a href="apps-tasks-details" class="fw-medium link-primary">#VLZ501</a>`
    );
};
    ...
    if (typeof ids_array !== "undefined" && ids_array.length > 0) {
        if (confirm("Are you sure you want to delete this?")) {
            Array.from(ids_array).forEach(function (id) {
                tasksList.remove(
                    "id",
                    `<a href="apps-tasks-details" class="fw-medium link-primary">${id}</a>`
                );
            });
        } else {
            return false;
