...
Array.from(document.querySelectorAll(".file-manager-menu a")).forEach(function (
    fileTab
) {
    fileTab.addEventListener("click", function () {
        var folderListelm = document.querySelector(
        ...
        if (
            ...
            var filterData = allFileList.filter(
                (filelist) => filelist.filetype === tabname
            );
            document.getElementById("folder-list").style.display = "none";
        } else if (tabname == "Important") {
            var filterData = allFileList.filter(
                (filelist) => filelist.starred == true
            );
            document.getElementById("folder-list").style.display = "none";
        } else {
...
Array.prototype.slice.call(createFolderForms).forEach(function (form) {
    form.addEventListener(
        ...
        <div class="card bg-light shadow-none" id="folder-' +
                    uniqueid +
                    '">\
            <div class="card-body">\
                <div class="d-flex mb-1">\
                    <div class="form-check form-check-danger mb-3 fs-15 flex-grow-1">\
                        <input class="form-check-input" type="checkbox" value="" id="folderlistCheckbox_' +
                    uniqueid +
                    '">\
                ...
                if (folderName !== "" && !editFlag) {
                    var folderListdata =
                        document.getElementById("folderlist-data");
                    folderListdata.insertAdjacentHTML(
                        "afterbegin",
                        folderlisthtml
                    );
                    var addFolderClose =
                        document.getElementById("addFolderBtn-close");
                    ...
                    document
                        .getElementById(getEditid)
                        .querySelector(".folder-name").innerHTML = folderName;
                    var addFolderClose =
                        document.getElementById("addFolderBtn-close");
                    addFolderClose.click();
                    editFlag = false;
...
function editFolderList() {
    Array.from(document.querySelectorAll(".folder-card")).forEach(function (
        item
    ) {
        Array.from(item.querySelectorAll(".edit-folder-list")).forEach(
            function (subitem) {
                subitem.addEventListener("click", function (event) {
                    ...
                    if (getEditid == getCheckid) {
                        ...
                        document.getElementById(
                            "createFolderModalLabel"
                        ).innerHTML = "Folder Rename";
                        document.getElementById("folderid-input").value =
                            "folder-" + getEditid;
                        document.getElementById("foldername-input").value =
                            item.querySelector(".folder-name").innerHTML;
                    }
                });
            }
        );
...
if (removeProduct) {
    removeProduct.addEventListener("show.bs.modal", function (e) {
        document
            .getElementById("remove-folderList")
...
Array.from(document.querySelectorAll(".createFile-modal")).forEach(function (
    elem
) {
    elem.addEventListener("click", function (event) {
...
Array.from(document.querySelectorAll(".create-folder-modal")).forEach(function (
    elem
) {
    elem.addEventListener("click", function (event) {
...
function removeSingleItem() {
    ...
    Array.from(document.querySelectorAll(".remove-list")).forEach(function (
        ...
        item.addEventListener("click", function (event) {
            ...
            document
                .getElementById("remove-fileitem")
                .addEventListener("click", function () {
                    ...
                    document.getElementById("file-overview").style.display =
                        "none";
                    document.getElementById("folder-overview").style.display =
                        "block";
                });
        });
    });
...
function fileDetailShow() {
    ...
    Array.from(document.querySelectorAll("#file-list tr")).forEach(function (
        ...
        item.querySelector(".viewfile-list").addEventListener(
            "click",
            function () {
                ...
                var filelistIcon =
                    item.querySelector(".filelist-icon i").className;
                var filelistName =
                    item.querySelector(".filelist-name").innerHTML;
                var filelistSize =
                    item.querySelector(".filelist-size").innerHTML;
                var filelistCreate =
                    item.querySelector(".filelist-create").innerHTML;
                var filelistType =
                    item.querySelector(".filelist-type").innerHTML;

                document.querySelector(
                    "#file-overview .file-icon i"
                ).className = filelistIcon;
                Array.from(
...
function removefileOverview() {
    var getid = 0;
    Array.from(document.querySelectorAll(".remove-file-overview")).forEach(
        function (item) {
            item.addEventListener("click", function (event) {
                ...
                document
                    ...
                    .addEventListener("click", function () {
                        ...
                        loadFileData(allFileList);
                        document
                            .getElementById("close-removefilemodal")
                            .click();
                        document
                            .getElementsByTagName("body")[0]
                            .classList.remove("file-detail-show");
                    });
            });
        }
    );
