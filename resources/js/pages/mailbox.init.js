...
Array.from(document.querySelectorAll(".mail-list a")).forEach(function (
    mailTab
) {
    mailTab.addEventListener("click", function () {
        ...
        if (
            mailTab.querySelector(".mail-list-link").hasAttribute("data-type")
        ) {
            var tabname = mailTab.querySelector(".mail-list-link").innerHTML;
            var filterData = allmaillist.filter(
                (maillist) => maillist.labeltype === tabname
            );
        } else {
            var tabname = mailTab.querySelector(".mail-list-link").innerHTML;
            ...
            if (tabname != "All") {
                var filterData = allmaillist.filter(
                    (maillist) => maillist.tabtype === tabname
                );
            } else {
...
function emailDetailShow() {
    ...
    Array.from(document.querySelectorAll(".message-list a")).forEach(function (
        item
    ) {
    ...
    Array.from(document.querySelectorAll(".close-btn-email")).forEach(function (
        item
    ) {
        item.addEventListener("click", function () {
    ...
    function checkAll() {
        ...
        if (checkedCount > 0) {
            Array.from(checkboxes).forEach(function (chkbox) {
                ...
                chkbox.parentNode.parentNode.parentNode.classList.remove(
                    "active"
                );
            });
        } else {
...
markAllReadBtn.addEventListener("click", function (event) {
    ...
    Array.from(document.querySelectorAll(".message-list li.unread")).forEach(
        function (element) {
            if (element.classList.contains("unread")) {
...
Array.from(document.querySelectorAll(".email-chat-list a")).forEach(function (
    item
) {
    item.addEventListener("click", function (event) {
    ...
    Array.from(document.querySelectorAll(".message-list li")).forEach(function (
        ...
        item.addEventListener("click", function () {
            ...
            Array.from(
                ...
                subitem
                    .querySelector("img")
                    .setAttribute("src", usermailProfile);
            });
        });
    });
}
