    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (cell, row) {
                var x = new DOMParser()
                    .parseFromString(
                        row._cells[0].data.props.content,
                        "text/html"
                    )
                    .body.querySelector(
                ...
                return gridjs.html(
                    '<div class="dropdown">' +
                        ...
                        '<li><a class="dropdown-item edit-list" data-edit-id=' +
                        x +
...
searchProductList.addEventListener("keyup", function () {
    ...
    function filterItems(arr, query) {
        return arr.filter(function (el) {
            return (
                el.product.title.toLowerCase().indexOf(query.toLowerCase()) !==
                -1
            );
        });
    }
...
Array.from(document.querySelectorAll(".filter-list a")).forEach(function (
    ...
    filteritem.addEventListener("click", function () {
        ...
        var filterData = productListAllData.filter(
            (filterlist) => filterlist.product.category === filterItemValue
        );
        var filterPublishedData = productListPublishedData.filter(
            (filterlist) => filterlist.product.category === filterItemValue
        );
...
Array.from(
    ...
    var isFilterSelected = item.querySelectorAll(
        ".filter-check .form-check .form-check-input:checked"
    ).length;
    item.querySelector(".filter-badge").innerHTML = isFilterSelected;
    Array.from(item.querySelectorAll(".form-check .form-check-input")).forEach(
        function (subitem) {
            ...
            subitem.addEventListener("click", function (event) {
                if (subitem.checked) {
                    isFilterSelected++;
                    item.querySelector(".filter-badge").innerHTML =
                        isFilterSelected;
                    isFilterSelected > 0
                        ? (item.querySelector(".filter-badge").style.display =
                              "block")
                        : (item.querySelector(".filter-badge").style.display =
                              "none");
                    filterChoicesInput.setValue([checkElm]);
...
searchBrandsOptions.addEventListener("keyup", function () {
    ...
    Array.from(searchItem).forEach(function (elem) {
        var searchBrandsTxt = elem
            .getElementsByClassName("form-check-label")[0]
            .innerText.toLowerCase();
        elem.style.display = searchBrandsTxt.includes(inputVal)
            ? "block"
            : "none";
    });
});
            ...
            .addEventListener("click", function () {
                Array.from(
                    ...
                    if (element.classList.contains("gridjs-tr-selected")) {
                        ...
                        var filteredPublished = arrayRemove(
